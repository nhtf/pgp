...
//	int tessMeshTessellateMonoRegion( TESSmesh *mesh, TESSface *face )
    private tessellateMonoRegion_( mesh, face ) {
        var up, lo;
    ...
    */
    ...
    private tessellateInterior_( mesh ) {
        var f, next;
...
//	int tessMeshSetWindingNumber( TESSmesh *mesh, int value, int keepOnlyBoundary )
    private setWindingNumber_( mesh, value, keepOnlyBoundary ) {
        var e, eNext;
    ...
    private getNeighbourFace_(edge)
    {
        if (!edge.Rface)
            return -1;
    ...
    private outputPolymesh_( mesh, elementType, polySize: number, vertexSize: number ) {
        var v;
        var f;
        var edge;
        var maxFaceCount = 0;
...
//	void OutputContours( TESStesselator *tess, TESSmesh *mesh, int vertexSize )
    private outputContours_( mesh, vertexSize: number ) {
        var f;
        var edge;
        var start;
        var verts;
...
//	int tessTesselate( TESStesselator *tess, int windingRule, int elementType, int polySize, int vertexSize, const TESSreal* normal )
    public tesselate( windingRule, elementType, polySize: number, vertexSize: number, normal ) {
        this.vertices = [];
        this.elements = [];
        this.vertexIndices = [];
