...
export class Racket extends Entity {
	controller: XRTargetRaySpace | null;

	constructor(controller: XRTargetRaySpace | null, mesh: Object3D) {
		const cylinderShape = createShape(mesh.children[0]);
		console.log(cylinderShape.getMargin());
		super(mesh, cylinderShape, 0.25, new Vector(0, 1, 0), new Quaternion(0, 0, 0, 1), true);
